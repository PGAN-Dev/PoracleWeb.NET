...
- **Important**: Area dual-writes are PoracleNG's responsibility only for admin areas. Custom geofence activate/deactivate writes directly to `humans.area` and the active `profiles.area` via `IHumanRepository` / `IProfileRepository`, bypassing `SetAreasAsync`. Going through `SetAreasAsync` is impossible for user geofences because of PoracleNG's `userSelectable=true`-only intersection filter for non-admins -- see "User geofence persistence" in Common Issues.
...
- **Creation**: `CreateAsync` stores the geofence in `user_geofences` and adds its `kojiName` to the **current** profile's area list, writing directly to both `humans.area` (via `IHumanRepository`) and `profiles.area` for `human.CurrentProfileNo` (via `IProfileRepository`). The geofence appears as active on the creating profile and inactive on all others.
- **Deletion**: `DeleteAsync` removes the geofence's `kojiName` from **all** profiles via direct `IHumanRepository` / `IProfileRepository` DB writes (both `humans.area` and every row in `profiles.area`), then deletes the `user_geofences` row and reloads Poracle geofences. `AdminDeleteAsync` does the same. Going through the proxy is not viable because PoracleNG's `setAreas` intersects with `userSelectable=true` fences only and would drop the write.
...
**Workaround**: All user-geofence area mutations (`CreateAsync`, `DeleteAsync`, `AddToProfileAsync`, `RemoveFromProfileAsync`, `AdminDeleteAsync`) write directly to `humans.area` + the active `profiles.area` via `IHumanRepository` / `IProfileRepository` instead of going through the proxy. In addition, `AreaController.UpdateAreas` calls `IUserGeofenceService.PreserveOwnedAreasInHumanAsync` after `SetAreasAsync` to re-add any user-owned geofence names from the submitted list via direct DB. Without these workarounds, saving areas or toggling a geofence would silently fail to persist — the UI would show optimistic success but a refresh would reveal the geofence is gone (regression from PR #88 v2.0.0, fixed in v2.4.1 by restoring the pre-migration direct-DB path for this specific case).
