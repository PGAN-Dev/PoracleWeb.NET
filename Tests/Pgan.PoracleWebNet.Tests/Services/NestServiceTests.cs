...
public class NestServiceTests
{
    ...
    // --- Duplicate-on-edit ---
    // PoracleNG dedups nest tracking by a natural key. When an edit changes a field in that key it INSERTS
    ...
    {
        var model = new Nest { Uid = 41 };
        this._proxy.Setup(p => p.CreateAsync("nest", "user1", It.IsAny<JsonElement>()))
            .ReturnsAsync(new TrackingCreateResult([42], 0, 0, 1));
        this._proxy.Setup(p => p.DeleteByUidAsync("nest", "user1", 41)).Returns(Task.CompletedTask);

        var result = await this._sut.UpdateAsync("user1", model);

        this._proxy.Verify(p => p.DeleteByUidAsync("nest", "user1", 41), Times.Once);
    ...
    public async Task UpdateAsyncStillSucceedsWhenDeletingTheSupersededRowFails()
    {
        ...
        this._proxy.Setup(p => p.CreateAsync("nest", "user1", It.IsAny<JsonElement>()))
            .ReturnsAsync(new TrackingCreateResult([42], 0, 0, 1));
        this._proxy.Setup(p => p.DeleteByUidAsync("nest", "user1", 41))
