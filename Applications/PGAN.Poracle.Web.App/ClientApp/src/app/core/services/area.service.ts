...
export class AreaService {
  ...
  getGeofencePolygons(): Observable<GeofenceData[]> {
    return this.http.get<{status: string, geofence: GeofenceData[]}>(`${this.config.apiHost}/api/areas/geofence`)
      .pipe(
  ...
  getMapUrl(areaName: string): Observable<string | null> {
    ...
    return this.http
      .get<{ url: string }>(`${this.config.apiHost}/api/areas/map/${encodeURIComponent(areaName)}`)
      .pipe(
        map(res => {
