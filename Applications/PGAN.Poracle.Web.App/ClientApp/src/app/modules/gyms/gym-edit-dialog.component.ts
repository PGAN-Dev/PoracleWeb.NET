...
@Component({
  selector: 'app-gym-edit-dialog', standalone: true,
  ...
  template: `
    <h2 mat-dialog-title>Edit Gym Alarm</h2>
    <mat-dialog-content>
      <div class="item-header"><span class="team-dot" [style.background]="getTeamColor(data.team)"></span><div><h3>{{ getTeamName(data.team) }}</h3><span class="item-subtitle">Gym Alarm</span></div></div>
  ...
  styles: [`
    ...
    .team-dot { width: 32px; height: 32px; border-radius: 50%; display: inline-block; }
...
export class GymEditDialogComponent {
  ...
  form = this.fb.group({
    ...
    slot_changes: [this.data.slot_changes === 1], battle_changes: [this.data.battle_changes === 1],
  ...
  save(): void {
    ...
    this.gymService.update(this.data.uid, { distance: dist, slot_changes: v.slot_changes ? 1 : 0, battle_changes: v.battle_changes ? 1 : 0, ping: v.ping || null, template: v.template || null, clean: v.clean ? 1 : 0 } as GymUpdate).subscribe({
      next: () => { this.snackBar.open('Gym alarm updated', 'OK', { duration: 3000 }); this.dialogRef.close(true); },
