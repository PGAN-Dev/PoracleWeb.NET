...
### Fixed
- **Raid/Egg add dialog only offered levels 1–6** ([#259](https://github.com/PGAN-Dev/PoracleWeb.NET/issues/259)): PoracleNG accepts arbitrary integer levels and users configuring higher levels via the bot's `!command` interface (e.g., Elite Raids at level 7+) saw them work correctly, but the UI's level checkboxes and boss-level dropdown were hardcoded to `[1, 2, 3, 4, 5, 6]`. Extended to `[1..10]` so the common Pokémon GO levels (1–5, Mega = 6, Elite ≈ 7) and a buffer for custom server schemes are reachable from the UI. One-line change in `raid-add-dialog.component.ts`; the edit dialog already displayed arbitrary existing levels read-only, and the backend imposes no upper bound.
