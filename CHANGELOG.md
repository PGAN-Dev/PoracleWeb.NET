...
### Security
- **Server-side enforcement of `disable_*` alarm-type settings** ([#236](https://github.com/PGAN-Dev/PoracleWeb.NET/issues/236)): the `disable_mons` / `disable_raids` / `disable_quests` / `disable_invasions` / `disable_lures` / `disable_nests` / `disable_gyms` / `disable_maxbattles` / `disable_fort_changes` site settings only hid navigation items in the Angular UI — every alarm controller was reachable via direct API calls regardless of the toggle, and several service-to-service paths (quick-pick apply, profile duplicate/import, cleaning toggle) wrote alarms past the controller layer entirely.

  **Defense-in-depth gating across three layers:**
  - **Constants & types** (`Core/Pgan.PoracleWebNet.Core.Models/`): `DisableFeatureKeys` (single source of truth for the 9 keys plus a tracking-type→key map), `FeatureDisabledException` (typed exception carrying the violated key).
  - **Gate service** (`Core/Pgan.PoracleWebNet.Core.Services/FeatureGate.cs`): new `IFeatureGate` exposes `IsEnabledAsync(key)` for boolean checks and `EnsureEnabledAsync(key)` that throws `FeatureDisabledException`. Logs the throw at Info level for an audit trail.
  - **Controller layer** (`Applications/Pgan.PoracleWebNet.Api/Filters/`): `RequireFeatureEnabledAttribute` resource filter on all 10 alarm controllers (`MonsterController`, `RaidController`, `EggController`, `QuestController`, `InvasionController`, `LureController`, `NestController`, `GymController`, `MaxBattleController`, `FortChangeController`) short-circuits with HTTP 403 before model binding. `TestAlertController` does the same check inline since the alarm type is a path parameter. `FeatureDisabledExceptionFilter` is registered globally to catch service-thrown `FeatureDisabledException` and map it to the same 403 response. Both response paths share `FeatureDisabledResponse.Create(disableKey)` so the wire format can't drift.
  - **Service layer**: every alarm service's `CreateAsync`, `BulkCreateAsync`, and `UpdateAsync` now calls `EnsureEnabledAsync` first — closes the bypass paths used by `QuickPickService.Apply` (which routes through the per-type alarm services), `CleaningService.ToggleCleanAsync` (which write through the proxy), and `ProfileOverviewService.DuplicateProfileAsync` / `ImportAlarmsAsync` (which iterate the raw payload). The two `ProfileOverview` methods also pre-validate the entire payload before any state mutation so a partial copy can't fail mid-loop and leave a half-populated profile.

  **Frontend follow-through:**
  - **`disabledFeatureGuard(disableKey)`** route guard factory applied to all 9 alarm routes — typing `/pokemon` or hitting a stale bookmark while `disable_mons=true` redirects to `/dashboard` with a clear toast instead of loading an empty page that 403s on every API call.
  - **403 interceptor** distinguishes "feature disabled" (response body has a `disableKey` field) from generic permission denials — the former triggers `ERROR.FEATURE_DISABLED` and a redirect to `/dashboard` so the user lands somewhere usable; the latter keeps the existing `ERROR.PERMISSION_DENIED` toast and stays on the page.
  - **Left nav** now hides disabled items for admins too (previously admins saw every nav item, which recreated the same UI/API mismatch in miniature). Empty alarm-group label and divider are also suppressed when no items remain.
  - **`ERROR.FEATURE_DISABLED`** translation added to all 11 locales (`en`, `da`, `de`, `es`, `fr`, `it`, `nl`, `pl`, `pt`, `pt-BR`, `sv`).

  **Performance:**
  - `SiteSettingService.GetByKeyAsync` is now wrapped in `IMemoryCache` with a 5-minute TTL and explicit invalidation on `CreateOrUpdateAsync` / `DeleteAsync`. Without this the dashboard would add 10 MySQL roundtrips per page load (one gate check per alarm-controller endpoint). Documented TOCTOU window: a slow read overlapping a write can leak stale data for up to the TTL — acceptable trade-off given the rarity of admin toggle changes.

  **Settings:** `disable_fort_changes` was missing from `SettingsMigrationService.BooleanKeys` and `CategoryMap` (pre-existing oversight) — now seeded so legacy `pweb_settings` migrations carry the value over.

  **Coverage:** ~25 new tests added — `FeatureGateTests`, `FeatureDisabledExceptionFilterTests`, `RequireFeatureEnabledAttributeTests`, per-service disable tests on `MonsterService` and `CleaningService` (representative; other 9 services share the gate via `IFeatureGate`), `ProfileOverviewService` pre-validation tests, `ProfileOverviewController` exception-propagation tests, frontend `disabled-feature.guard.spec.ts` (including the loadOnce-fails fallback), and 403-with-disableKey case in `error.interceptor.spec.ts`.

  **Operator note:** the toggle now means "nobody uses this feature" — including admins. If an admin needs to verify a disabled feature works, they flip the toggle off in Admin → Settings. `disable_eggs` does not exist; eggs share `disable_raids` because they share the raid UI.
