...
### Fixed
- **Mixed and Decoy grunt gender variants** ([#221](https://github.com/PGAN-Dev/PoracleWeb.NET/issues/221)): the Mixed and Decoy entries in the add-invasion dialog were single checkboxes with one icon each, hiding the fact that each has male/female variants with distinct invasion character IDs (Mixed Male 4 / Female 5; Decoy Male 45 / Female 46). The dialog now offers separate Male/Female checkboxes per variant, and `getGruntIconUrl` / `getDisplayName` use the alarm's `gender` to render the correct icon and a "(Male)"/"(Female)" suffix in the list and edit views.
