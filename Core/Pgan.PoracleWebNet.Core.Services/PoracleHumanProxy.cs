...
public class PoracleHumanProxy(HttpClient httpClient, IConfiguration configuration) : IPoracleHumanProxy
{
    ...
    {
        var response = await this.SendAsync(HttpMethod.Get, $"/api/humans/one/{userId}");
    ...
    {
        var response = await this.SendAsync(HttpMethod.Post, $"/api/humans/{userId}/start");
        response.EnsureSuccessStatusCode();
    }

    public async Task StopAsync(string userId)
    {
        var response = await this.SendAsync(HttpMethod.Post, $"/api/humans/{userId}/stop");
        response.EnsureSuccessStatusCode();
    }
    ...
    public async Task AdminDisabledAsync(string userId, bool disabled)
    {
        ...
        var response = await this.SendAsync(HttpMethod.Post, $"/api/humans/{userId}/adminDisabled", body);
        response.EnsureSuccessStatusCode();
    }

    public async Task SetLocationAsync(string userId, double lat, double lon)
    {
        var response = await this.SendAsync(HttpMethod.Post, $"/api/humans/{userId}/setLocation/{lat}/{lon}");
        response.EnsureSuccessStatusCode();
    }
    ...
    public async Task SetAreasAsync(string userId, string[] areas)
    {
        ...
        var response = await this.SendAsync(HttpMethod.Post, $"/api/humans/{userId}/setAreas", body);
        response.EnsureSuccessStatusCode();
    }
    ...
    {
        var response = await this.SendAsync(HttpMethod.Post, $"/api/humans/{userId}/switchProfile/{profileNo}");
        response.EnsureSuccessStatusCode();
    }

    public async Task<JsonElement> GetProfilesAsync(string userId)
    {
        var response = await this.SendAsync(HttpMethod.Get, $"/api/profiles/{userId}");
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await this.SendAsync(HttpMethod.Post, $"/api/profiles/{userId}/add", body.GetRawText());
        response.EnsureSuccessStatusCode();
    }

    public async Task UpdateProfileAsync(string userId, JsonElement body)
    {
        var response = await this.SendAsync(HttpMethod.Post, $"/api/profiles/{userId}/update", body.GetRawText());
        response.EnsureSuccessStatusCode();
    }

    public async Task DeleteProfileAsync(string userId, int profileNo)
    {
        var response = await this.SendAsync(HttpMethod.Delete, $"/api/profiles/{userId}/byProfileNo/{profileNo}");
        response.EnsureSuccessStatusCode();
    }

    public async Task<JsonElement?> CheckLocationAsync(string userId, double lat, double lon)
    {
        var response = await this.SendAsync(HttpMethod.Get, $"/api/humans/{userId}/checkLocation/{lat}/{lon}");
        if (!response.IsSuccessStatusCode)
        {
            return null;
        }
